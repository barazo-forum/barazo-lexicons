...
describe("topicPostSchema", () => {
  ...
  it("rejects empty title", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, title: "" }).success,
    ).toBe(false);
  });
  ...
  it("rejects title exceeding maxLength (2000 bytes)", () => {
    ...
    expect(
      topicPostSchema.safeParse({ ...validPost, title: longTitle }).success,
    ).toBe(false);
  });

  it("rejects empty content", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, content: "" }).success,
    ).toBe(false);
  });
  ...
  it("rejects content exceeding maxLength (100000)", () => {
    ...
    expect(
      topicPostSchema.safeParse({ ...validPost, content: longContent }).success,
    ).toBe(false);
  });

  it("rejects invalid DID format for community", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, community: "not-a-did" })
        .success,
    ).toBe(false);
  });
  ...
  it("rejects empty tag strings", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, tags: [""] }).success,
    ).toBe(false);
  });

  it("rejects invalid datetime format", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, createdAt: "not-a-date" })
        .success,
    ).toBe(false);
  });
  ...
  it("rejects empty category", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, category: "" }).success,
    ).toBe(false);
  });
  ...
  it("rejects missing required fields", () => {
    ...
    expect(
      topicPostSchema.safeParse({ title: "Test" }).success,
    ).toBe(false);
  });

  it("rejects invalid contentFormat", () => {
    expect(
      topicPostSchema.safeParse({ ...validPost, contentFormat: "html" })
        .success,
    ).toBe(false);
  });
...
describe("reactionSchema", () => {
  ...
  it("rejects empty type string", () => {
    expect(
      reactionSchema.safeParse({ ...validReaction, type: "" }).success,
    ).toBe(false);
  });

  it("rejects type exceeding 300 bytes", () => {
    expect(
      reactionSchema.safeParse({ ...validReaction, type: "x".repeat(301) })
        .success,
    ).toBe(false);
  });
...
describe("actorPreferencesSchema", () => {
  ...
  it("rejects more than 1000 blocked DIDs", () => {
    const tooMany = {
      ...validPrefs,
      blockedDids: Array.from(
        { length: 1001 },
        (_, i) => `did:plc:blocked${String(i)}`,
      ),
    };
    expect(actorPreferencesSchema.safeParse(tooMany).success).toBe(false);
  });
